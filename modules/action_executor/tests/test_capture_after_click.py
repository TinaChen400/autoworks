...
def test_capture_after_click_uses_latest_capture_provenance_without_restore(
    ...
    provenance = {
        ...
        "locked_region": {"left": 100, "top": 80, "width": 4, "height": 3},
        "dpi_scale": 1.5,
    }
    ...
    monkeypatch.setattr(
        ...
        lambda _hwnd: WindowPlacement(left=100, top=80, width=4, height=3),
    )
...
def test_capture_after_click_fails_if_locked_target_moved(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        capture_after_click,
        "get_window_placement",
        lambda _hwnd: WindowPlacement(left=100, top=80, width=5, height=3),
    )
