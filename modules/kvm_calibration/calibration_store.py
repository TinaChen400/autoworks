...
def paths_for_runtime(runtime_dir: str | Path) -> tuple[Path, Path]:
    ...
    return (
        runtime / "latest_kvm_calibration.json",
        runtime / "latest_kvm_calibration_report.json",
    )
...
def load_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    return (
        ...
        load_json(ACTION_EXECUTOR_PREVIEW_PATH),
    )


def save_calibration(
    calibration: dict[str, Any],
    path: str | Path = KVM_CALIBRATION_PATH,
) -> Path:
    return save_json(path, calibration)


def save_report(report: dict[str, Any], path: str | Path = KVM_CALIBRATION_REPORT_PATH) -> Path:
